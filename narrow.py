...
class randomSample:
    ...
    def __init__(self):
        ...
        weights_1 = int(0.6*self.curValue.sample_size)

        self.x = np.random.uniform(-70, 70, weights_1)
        self.y = np.random.uniform(-70, 70, weights_1)

        weights_2 = int(0.4*self.curValue.sample_size)


        self.weight_x = np.random.uniform(-40, 40, weights_2)
        self.weight_y = np.random.uniform(-4, 4, weights_2)

        for each_x in self.weight_x:

            self.x = np.append(self.x, each_x)

        for each_y in self.weight_y:

            self.y = np.append(self.y, each_y)
...
class obstacle:
    ...
    def __init__(self):
        ...
        self.all_lines.append([[-25,70,25,70],[-25,70,-25,5],[-25,5,25,5],[25,70,25,5]])
        self.all_lines.append([[-25,-70,25,-70],[-25,-70,-25,-5],[25,-5,-25,-5],[25,-70,25,-5]])
    ...
    def val(self,index):
        ...
        if index == 1:
            ...
            pts = np.array([[-25,70], [25,70], [-25,5], [25,5]])
...
class CarSimulator:
    ...
    def motion(self):
        ...
        plt.plot(-65, 35, marker=".", markersize=12, color = 'r')

        plt.plot(58, -48, marker=".", markersize=12, color = 'lightgreen')

        #jjj_xx,jjj_yy = self.sample.val()

        #plt.scatter(jjj_xx,jjj_yy)
